...
export async function GET(
  ...
  try {
    ...
    const partialResponses = responses.filter(r => r.status === 'partial').length
    ...
    const averageTime = responsesWithTime.length > 0 
      ? responsesWithTime.reduce((sum, r) => sum + (r.timeSpent || 0), 0) / responsesWithTime.length
      : 0
    ...
    // Device analytics
    const deviceStats = responses.reduce((acc, response) => {
    ...
    const fieldAnalytics = form.fields.map((field: any) => {
      ...
      if (['multiple-choice', 'dropdown', 'checkbox', 'yes-no'].includes(field.type)) {
        const valueDistribution = fieldResponses.reduce((acc, response: any) => {
        ...
        analytics.valueDistribution = Object.entries(valueDistribution)
          .map(([value, count]) => ({ value, count, percentage: Math.round((count / responseCount) * 100) }))
      ...
      if (['rating', 'linear-scale', 'number'].includes(field.type)) {
        ...
        if (values.length > 0) {
          analytics.average = Math.round((values.reduce((sum, v) => sum + v, 0) / values.length) * 100) / 100
      ...
      if (form.settings.assignmentMode && field.correctAnswer !== undefined) {
        const correctResponses = fieldResponses.filter((r: any) => r.isCorrect).length
        analytics.correctRate = responseCount > 0 ? Math.round((correctResponses / responseCount) * 100) : 0
      }
    ...
    let scoringAnalytics = null
    if (form.settings.assignmentMode) {
      const scoredResponses = responses.filter(r => r.totalScore !== undefined && r.maxScore !== undefined)
      
      if (scoredResponses.length > 0) {
        const scores = scoredResponses.map(r => (r.totalScore! / r.maxScore!) * 100)
        ...
        scoringAnalytics = {
          ...
          passingRate: Math.round((scores.filter(s => s >= (form.settings.passingScore || 70)).length / scores.length) * 100),
    ...
    return NextResponse.json({
      ...
      demographics: {
        devices: Object.entries(deviceStats.devices).map(([device, count]) => ({ device, count })),
        browsers: Object.entries(deviceStats.browsers).map(([browser, count]) => ({ browser, count })),
        operatingSystems: Object.entries(deviceStats.operatingSystems).map(([os, count]) => ({ os, count })),
